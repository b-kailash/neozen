...
import shlex # Import shlex for safer argument checking
...
from PyQt6.QtWidgets import (
    ...
    QAbstractItemView # Added for selection behavior
)
...
class MainWindow(QMainWindow):
    ...
    Includes Host Details tab and privilege warnings in status bar.
    Uses QFont.Monospace for better cross-platform font handling.
    """
    PRIVILEGED_FLAGS = ['-sS', '-sU', '-O', '-A'] # Nmap flags often requiring root/admin
    ...
    def __init__(self):
        ...
        self.current_scan_data = {} # Store full parsed data for details view
        self._current_status_message = "Ready" # Store base status message

        self.setWindowTitle("NeoZen - Modern Nmap GUI")
        self.setGeometry(100, 100, 950, 800) # Increased height slightly

        self._create_actions()
        self._create_menu_bar()
        self._create_central_widget()
        self._create_status_bar() # Create status bar with label
        self._connect_signals()

        # Initial check after UI is built and profiles/scan types loaded
        self._check_and_warn_privileged_scan() # Update status label initially
    ...
    def _create_central_widget(self):
        """Create the central widget and its layout, including Host Details tab."""
        central_widget = QWidget()
        self.setCentralWidget(central_widget)
        main_layout = QVBoxLayout(central_widget)

        # --- Top Row & Config (Identical to previous version) ---
        ...
        monospace_font = QFont("Monospace") # Use Qt's generic family name
        monospace_font.setStyleHint(QFont.StyleHint.TypeWriter) # Further hint for monospacing
        ...
        self.output_area.setFont(monospace_font) # Use defined monospace font
        ...
        # Host Details Tab
        self.host_details_widget = QWidget()
        host_details_layout = QVBoxLayout(self.host_details_widget)
        host_details_layout.setContentsMargins(5, 5, 5, 5)
        self.host_details_area = QTextEdit()
        self.host_details_area.setReadOnly(True)
        self.host_details_area.setFont(monospace_font) # Use defined monospace font
        self.host_details_area.setLineWrapMode(QTextEdit.LineWrapMode.NoWrap)
        host_details_layout.addWidget(self.host_details_area)
        self.tab_widget.addTab(self.host_details_widget, "Host Details")
    ...
    def _create_status_bar(self):
        """Creates the status bar and adds a permanent label widget."""
    ...
    def _check_and_warn_privileged_scan(self):
        """Checks current arguments and updates status label with privilege warning."""
        # (Code remains the same)
        args_string = self.custom_args_input.text()
        try:
            args_list = shlex.split(args_string)
        except ValueError:
            args_list = args_string.split() # Fallback
        ...
        if needs_privileges:
            ...
            self.status_label.setText(self._current_status_message + warning_text)
        else:
    ...
    def update_args_from_scan_type(self):
        """Updates the custom arguments input and checks for privileges."""
        # (Code remains the same)
    ...
    def load_profile_settings(self):
        """Loads settings from profile and checks for privileges."""
        # (Code remains the same)
        selected_profile_name = self.profile_combo.currentText(); is_custom = (selected_profile_name == "Custom Scan")
        if is_custom:
            self.scan_type_combo.setCurrentIndex(0)
        elif selected_profile_name in self.profiles:
            profile_data = self.profiles[selected_profile_name]; self.target_input.setText(profile_data.get("target", "")); saved_args = profile_data.get("arguments", ""); found_match = False
            for i in range(self.scan_type_combo.count()):
                if self.scan_type_combo.itemData(i, Qt.ItemDataRole.UserRole) == saved_args: self.scan_type_combo.setCurrentIndex(i); found_match = True; break
            if not found_match:
                 self.custom_args_input.setText(saved_args)
                 self.custom_args_input.setReadOnly(False)
                 self.custom_args_input.setToolTip("Custom arguments loaded from profile")
                 self._check_and_warn_privileged_scan() # Check custom args
        else:
             QMessageBox.warning(self, "Profile Error", f"Could not load data for profile '{selected_profile_name}'."); self.profile_combo.setCurrentIndex(0)
        self.delete_profile_button.setEnabled(not is_custom and selected_profile_name in self.profiles)


    def save_current_profile(self):
        """Saves the current UI settings as a new profile."""
        # (Code remains the same)
        current_args = self.custom_args_input.text().strip(); current_target = self.target_input.text().strip(); dialog = SaveProfileDialog(self)
        if dialog.exec() == QDialog.DialogCode.Accepted:
            profile_name = dialog.get_profile_name()
            if not profile_name or profile_name == "Custom Scan": QMessageBox.warning(self, "Save Error", "Profile name cannot be empty or 'Custom Scan'."); return
            profile_data = {"arguments": current_args, "target": current_target}; is_update = profile_name in self.profiles; self.profiles[profile_name] = profile_data
            if self.profile_manager.save_profiles(self.profiles):
                ...
                if not is_update:
                    ...
                    self.profiles = self.profile_manager.load_profiles(); self.profile_combo.clear(); self.profile_combo.addItem("Custom Scan"); self.profile_combo.addItems(sorted(self.profiles.keys()))
                    current_index = self.profile_combo.findText(profile_name); self.profile_combo.setCurrentIndex(current_index if current_index >= 0 else 0)
    ...
    def delete_selected_profile(self):
        ...
        if selected_profile_name == "Custom Scan" or selected_profile_name not in self.profiles: QMessageBox.warning(self, "Delete Error", "Cannot delete 'Custom Scan' or non-existent profile."); return
        reply = QMessageBox.question(self, "Confirm Delete", f"Delete profile '{selected_profile_name}'?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
        if reply == QMessageBox.StandardButton.Yes:
            original_profile_data = self.profiles.pop(selected_profile_name, None)
            if self.profile_manager.save_profiles(self.profiles):
                ...
                self.profile_combo.setCurrentIndex(0)
                 ...
                 if original_profile_data: self.profiles[selected_profile_name] = original_profile_data
                 self.profile_combo.clear(); self.profile_combo.addItem("Custom Scan"); self.profile_combo.addItems(sorted(self.profiles.keys())); self.profile_combo.setCurrentIndex(0)


    # --- Scan Methods ---
    def get_nmap_arguments(self): return self.custom_args_input.text().strip()
    ...
    def start_scan(self):
        ...
        if not target: QMessageBox.warning(self, "Missing Target", "Please enter a target to scan."); return
        if self.scanner_thread and self.scanner_thread.isRunning(): QMessageBox.information(self, "Scan in Progress", "A scan is already running."); return
    ...
    def process_scan_results(self, results_data):
        """Stores the full parsed data and triggers table display."""
        # (Code remains the same)
        self.current_scan_data = results_data
        self.display_parsed_results_table(results_data)
        self.host_details_area.clear()
    ...
    def display_parsed_results_table(self, results_data):
        ...
        # (Code remains the same)
        self.results_table.setSortingEnabled(False); self.results_table.setRowCount(0); row_position = 0
        for host, host_data in results_data.items():
            hostname = host_data.get('hostname', ''); display_host = f"{hostname} ({host})" if hostname and hostname != host else host
            protocols = host_data.get('protocols', {})
            if not protocols:
                if host_data.get('state') == 'up':
                     self.results_table.insertRow(row_position); host_item = QTableWidgetItem(display_host); host_item.setData(Qt.ItemDataRole.UserRole, host); self.results_table.setItem(row_position, 0, host_item)
                     self.results_table.setItem(row_position, 3, QTableWidgetItem(host_data.get('state', 'unknown'))); self.results_table.setItem(row_position, 4, QTableWidgetItem("(No ports found/reported)")); row_position += 1
                continue
            for proto, ports in protocols.items():
                for port, port_data in ports.items():
                    self.results_table.insertRow(row_position); host_item = QTableWidgetItem(display_host); host_item.setData(Qt.ItemDataRole.UserRole, host); self.results_table.setItem(row_position, 0, host_item)
                    self.results_table.setItem(row_position, 1, QTableWidgetItem(proto)); self.results_table.setItem(row_position, 2, QTableWidgetItem(str(port))); self.results_table.setItem(row_position, 3, QTableWidgetItem(port_data.get('state', '')))
                    self.results_table.setItem(row_position, 4, QTableWidgetItem(port_data.get('name', ''))); self.results_table.setItem(row_position, 5, QTableWidgetItem(port_data.get('product', ''))); self.results_table.setItem(row_position, 6, QTableWidgetItem(port_data.get('version', '')))
                    row_position += 1
        self.results_table.setSortingEnabled(True)
        if results_data: self.tab_widget.setCurrentWidget(self.parsed_results_widget)

    def display_host_details(self):
        """Displays detailed information for the selected host."""
        # (Code remains the same)
        selected_items = self.results_table.selectedItems(); self.host_details_area.clear()
        if not selected_items: return
        selected_row = self.results_table.currentRow(); host_item = self.results_table.item(selected_row, 0)
        if not host_item: return
        host_ip = host_item.data(Qt.ItemDataRole.UserRole)
        if host_ip and host_ip in self.current_scan_data:
            details = self.current_scan_data[host_ip]; details_text = f"Host Details: {host_ip}\n"; details_text += f"Hostname: {details.get('hostname', 'N/A')}\n"; details_text += f"State: {details.get('state', 'N/A')}\n"
            mac = details.get('mac', '');
            if mac: details_text += f"MAC Address: {mac}"; vendor = details.get('vendor', '');
            if vendor: details_text += f" ({vendor})"; details_text += "\n"
            osmatches = details.get('osmatch', [])
            if osmatches:
                details_text += "\n--- OS Detection ---\n"
                for match in sorted(osmatches, key=lambda x: int(x.get('accuracy', '0')), reverse=True):
                    details_text += f"  Name: {match.get('name')} (Accuracy: {match.get('accuracy')} %)\n"
                    for osclass in match.get('osclasses', []):
                         details_text += f"    Class: {osclass.get('vendor', '')} {osclass.get('osfamily', '')} {osclass.get('osgen', '')} (Type: {osclass.get('type', '')}, Acc: {osclass.get('accuracy')}%)\n"
            else:
                 details_text += "\n--- OS Detection: No match found or OS detection not performed ---\n"
            protocols = details.get('protocols', {})
            if protocols:
                details_text += "\n--- Ports/Services ---\n"
                for proto, ports in sorted(protocols.items()):
                    details_text += f"  Protocol: {proto}\n"
                    for port, port_data in sorted(ports.items()):
                        ...
                        details_text += f"    {port:<5} {state:<10} {name:<15}"
                        if prod: details_text += f" {prod}"
                        if ver: details_text += f" (version {ver})"
                        if extra: details_text += f" ({extra})"
                        details_text += "\n"
            else:
                if details.get('state') == 'up':
                     details_text += "\n--- Ports/Services: No open/reported ports found ---\n"
            self.host_details_area.setText(details_text)
        else:
    ...
    def _set_ui_scan_state(self, scanning: bool):
        ...
        # (Code remains the same)
        self.scan_button.setEnabled(not scanning); self.stop_button.setEnabled(scanning); self.progress_bar.setVisible(scanning)
        self.profile_combo.setEnabled(not scanning); self.scan_type_combo.setEnabled(not scanning); self.custom_args_input.setEnabled(not scanning)
        self.save_profile_button.setEnabled(not scanning); self.delete_profile_button.setEnabled(not scanning and self.profile_combo.currentIndex() > 0)
        self.save_action.setEnabled(not scanning and bool(self.last_scan_xml_path and os.path.exists(self.last_scan_xml_path))); self.open_action.setEnabled(not scanning)
    ...
    def _reset_ui_after_scan(self, status_message):
        ...
        self._check_and_warn_privileged_scan() # Update status label
        self.scanner_thread = None
    ...
    def open_scan_results(self):
        ...
        # (Code remains the same)
        if self.scanner_thread and self.scanner_thread.isRunning(): QMessageBox.warning(self, "Scan Running", "Cannot open results while a scan is in progress."); return
        filename, _ = QFileDialog.getOpenFileName(self, "Open Nmap Scan Results", "", "Nmap XML Files (*.xml);;All Files (*)")
        if filename:
            self._cleanup_last_scan_file(); self.current_scan_data = {}; self.host_details_area.clear(); self._current_status_message = f"Opening {filename}..."; self._check_and_warn_privileged_scan()
            try:
                with open(filename, 'r', encoding='utf-8') as f: xml_content = f.read()
                if not xml_content: QMessageBox.warning(self, "Empty File", f"The file '{filename}' is empty."); self._current_status_message = "Failed to open empty file."; self._check_and_warn_privileged_scan(); return
                nm = nmap.PortScanner(); scan_data = nm.analyse_nmap_xml_scan(nmap_xml_output=xml_content); parsed_results = {}
                if 'scan' in scan_data and isinstance(scan_data['scan'], dict):
                     for host_ip, host_scan_data in scan_data['scan'].items():
                          ...
                          if mac_address and 'vendor' in host_scan_data: vendor = host_scan_data['vendor'].get(mac_address, '')
                          host_data_for_ui = {'hostname': hostname, 'state': state, 'mac': mac_address, 'vendor': vendor, 'osmatch': [], 'protocols': {}}
                          ...
                          for proto in ['tcp', 'udp', 'ip', 'sctp']:
                               if proto in host_scan_data and isinstance(host_scan_data[proto], dict):
                                    ...
                                    for port_str, port_data in host_scan_data[proto].items():
                                         ...
                                         try: port_int = int(port_str); host_data_for_ui['protocols'][proto][port_int] = {'state': port_data.get('state', 'unknown'), 'name': port_data.get('name', ''),'version': port_data.get('version', ''), 'product': port_data.get('product', ''),'extrainfo': port_data.get('extrainfo', ''), 'cpe': port_data.get('cpe', '')}
                                         except (ValueError, TypeError): continue
                          parsed_results[host_ip] = host_data_for_ui
                else:
                     host_info = "(No host details found in XML)"
                     if 'nmap' in scan_data and 'hosts' in scan_data['nmap']: num_hosts = scan_data['nmap']['hosts'].get('up', 0); total_hosts = scan_data['nmap']['hosts'].get('total', 0); host_info = f"({num_hosts} host(s) up / {total_hosts} total)"
                     QMessageBox.information(self, "Scan Info", f"Loaded file contains scan summary but no detailed host results {host_info}.")
                self.current_scan_data = parsed_results; self.output_area.setText(f"--- Results loaded from: {filename} ---\n\n(Raw output not available for loaded files)"); self.display_parsed_results_table(parsed_results)
                self._current_status_message = f"Successfully loaded results from {filename}"; self._check_and_warn_privileged_scan(); self.last_scan_xml_path = None; self.save_action.setEnabled(False)
            except FileNotFoundError: QMessageBox.critical(self, "Error", f"File not found: {filename}"); self._current_status_message = "Error opening file."; self._check_and_warn_privileged_scan()
            except Exception as e: QMessageBox.critical(self, "Error Opening/Parsing File", f"Could not open or parse {filename}:\n{e}"); self._current_status_message = f"Error opening/parsing file: {e}"; self._check_and_warn_privileged_scan()

    def save_scan_results(self):
        """Saves the XML results of the last completed scan to a file."""
        # (Code remains the same)
        if not self.last_scan_xml_path or not os.path.exists(self.last_scan_xml_path): QMessageBox.warning(self, "No Results", "No scan results available to save."); return False
        suggested_filename = "nmap_scan_results.xml"; filename, _ = QFileDialog.getSaveFileName(self, "Save Nmap Scan Results", suggested_filename, "Nmap XML Files (*.xml);;All Files (*)")
        if filename:
            try: shutil.copyfile(self.last_scan_xml_path, filename); self._current_status_message = f"Scan results saved to {filename}"; self._check_and_warn_privileged_scan(); return True
            except Exception as e: QMessageBox.critical(self, "Save Error", f"Could not save results to {filename}:\n{e}"); self._current_status_message = f"Error saving results: {e}"; self._check_and_warn_privileged_scan(); return False
        else: self._current_status_message = "Save cancelled."; self._check_and_warn_privileged_scan(); return False

    def show_about_dialog(self):
        """Displays a simple About dialog."""
        # (Code remains the same)
        QMessageBox.about(self, "About NeoZen", "NeoZen - A Modern Nmap GUI\n\nBuilt with Python and PyQt.")
    ...
    def closeEvent(self, event):
        ...
        should_close = True
        if self.scanner_thread and self.scanner_thread.isRunning():
            reply = QMessageBox.question(self, 'Scan in Progress', "Scan running. Stop and exit?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
            if reply == QMessageBox.StandardButton.Yes: self.stop_scan();
            if self.scanner_thread: self.scanner_thread.wait(1500)
            else: should_close = False; event.ignore(); return
        if should_close and self.last_scan_xml_path and os.path.exists(self.last_scan_xml_path):
            msg_box = QMessageBox(self); msg_box.setWindowTitle("Unsaved Scan Results"); msg_box.setText("The results of the last scan have not been saved."); msg_box.setInformativeText("Do you want to save the results before exiting?")
            save_button = msg_box.addButton("&Save", QMessageBox.ButtonRole.AcceptRole); discard_button = msg_box.addButton("&Don't Save", QMessageBox.ButtonRole.DestructiveRole); cancel_button = msg_box.addButton("Cancel", QMessageBox.ButtonRole.RejectRole)
            msg_box.setDefaultButton(save_button); msg_box.setIcon(QMessageBox.Icon.Question); msg_box.exec(); clicked_button = msg_box.clickedButton()
            if clicked_button == save_button:
                if not self.save_scan_results(): should_close = False; event.ignore(); return
            elif clicked_button == discard_button: should_close = True
            else: should_close = False; event.ignore(); return
        if should_close: self._cleanup_last_scan_file(); event.accept()
        else: event.ignore()
